...
// Truy vấn giá trị rmq trong đoạn [qlow, qhigh]
int query(const vector<int>& segmentTree, int low, int high, int qlow, int qhigh, int pos) {
    // Đoạn truy vấn hoàn toàn nằm ngoài đoạn hiện tại
    if (qlow > high || qhigh < low)
        return INT_MAX;

    // Đoạn hiện tại hoàn toàn nằm trong đoạn truy vấn
    if (qlow <= low && qhigh >= high)
        return segmentTree[pos];

    int mid = (low + high) / 2;
    int left = query(segmentTree, low, mid, qlow, qhigh, 2 * pos + 1);
    int right = query(segmentTree, mid + 1, high, qlow, qhigh, 2 * pos + 2);
...
int main() {
    ...
    // Xây dựng Segment Tree with size of 2 * n - 1
    ...
    while(m-- != 0) {
        ...
        int rmq = query(segmentTree, 0, n - 1, start, end, 0);
        totalRmq += rmq;
    }
